...
//	puts("■ 不能输入空格字符。\n");
//
//	make4digits(no,n);					/* 生成n个数字各不相同的数字串 */
//
//	start = clock();					/* 开始计算 */
//
//	do {
...
//			scanf("%s", buff);			/* 读取为字符串 */
//
//			chk = check(buff,n);			/* 检查读取到的字符串 */
//
//			switch (chk) {
//			case 1: printf("\a请确保输入%d个字符。",n); break;
...
//
//		try_no++;
//		judge(buff, no, &hit, &blow,n);	/* 判断 */
//		print_result(hit + blow, hit,n);	/* 显示判断结果 */
//
//	} while (hit < n);
//
//	end = clock();						/* 计算结束 */
